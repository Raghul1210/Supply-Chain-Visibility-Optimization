...
Rename the main query to Fact_table.
Remove sensitive/unneeded columns: Customer Email, Customer Password, Order Zipcode.
...
In Fact_table, go to Home → Merge Queries.
Top table: Fact_table; bottom table: Dim_Shipping (or Dim_Location).
Select all matching common columns (held with Ctrl) as the join key.
Join kind: Left Outer.
Expand the resulting merged column and keep only the surrogate key (Shipping_id / Location_id) — dropping the rest to avoid duplicating attribute columns already living in the dimension.
